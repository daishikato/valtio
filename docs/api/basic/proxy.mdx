...
Subscribers are notified synchronously, once per change. Wrap several changes in [`batch`](../advanced/batch) to notify subscribers once.

```js
import { batch, proxy } from 'valtio'
